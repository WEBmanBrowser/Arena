...
describe("S35.3 catalog enrichment discovery",()=>{
  ...
  it("uses exact EAN matching and no fuzzy product-name matching",()=>{ const s=read("src/lib/catalog-enrichment/providers/upcitemdb.ts"); expect(s).toContain("NO_EXACT_EAN_MATCH"); expect(s).toContain("compactEan(i.ean)"); expect(s).not.toContain("similarity"); expect(s).not.toContain("levenshtein"); });
