...
export async function grantWebhookRecoveryBudget(input: {
  ...
  const grants = recoveryGrants(current);
  if (grants >= MAX_RECOVERY_GRANTS) return { outcome: "rejected", code: "GRANT_LIMIT_REACHED" };

  const [row] = await executor
    .update(providerWebhookEvents)
    .set({
      updatedAt: new Date(),
      metadata: sql`coalesce(${providerWebhookEvents.metadata}, '{}'::jsonb) || ${JSON.stringify({ recoveryGrants: grants + 1 })}::jsonb`,
    })
    .where(
      and(
  ...
  if (!row) return { outcome: "rejected", code: "CONFLICT" };
  return { outcome: "granted", event: row, grants: grants + 1 };
}
