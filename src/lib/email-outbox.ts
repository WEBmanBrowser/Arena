...
export async function requeueEmailNotification(input: {
  ...
  const [row] = await db
    ...
    .where(
      and(
        ...
        inArray(emailNotifications.status, ["delivery_unknown", "failed"])
      )
    )
    .returning();

  if (!row) return { ok: false, code: "NOT_REQUEUEABLE" };
  ...
  await createAuditLog({
    ...
    entityId: row.id,
    details: { type: row.type, referenceType: row.referenceType, referenceId: row.referenceId },
  });
