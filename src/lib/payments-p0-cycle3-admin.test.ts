...
describe("C6 — anomaly resolution requires an explicit classification", () => {
  ...
  it("refuses `REFUNDED` for an unattributable observation, even with a succeeded refund (F-3 fail closed)", async () => {
    const fixture = await createPendingOrder({ prefix: PREFIX });
    const resolver = await createUser(PREFIX, "manager");
    ...
    // An INGESTED observation: `ingestReconciliationObservation` never writes a
    // `paymentId`, so this anomaly cannot be attributed to ONE payment.
    const observation = await openAnomalyForOrder(fixture.orderId, resolver);
    expect(observation.paymentId).toBeNull();

    // A fully concluded 50.00 refund exists for this order — but binding it to
    // this observation would mean ASSUMING it belongs to the right payment. F-3
    // refuses that assumption instead of falling back to `orderId`.
    ...
    // The anomaly stays OPEN and carries no resolution at all. The attributable
    // acceptance path is covered by payments-p0-cycle4-f3.test.ts.
    const stillOpen = (
      await db
        .select()
        .from(reconciliationObservations)
        .where(eq(reconciliationObservations.id, observation.id))
        .limit(1)
    )[0];
    expect(stillOpen.status).toBe("open");
