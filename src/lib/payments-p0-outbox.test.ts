...
describe("M5 — operator surfaces", () => {
  ...
  it("requires manager+ to read and admin+CSRF to requeue", async () => {
    ...
    const okBody = (await ok.json()) as { outcome: string; dispatch: string };
    expect(okBody.outcome).toBe("requeued");

    // MEDIUM-2 — the requeue DISPATCHES. The suite deletes EMAIL_API_KEY, so the
    // one permitted dispatch is definitive ("no transport configured") and the
    // operator is told exactly that. What matters is that the row did NOT stay
    // parked in `queued` waiting for a sweep nobody calls: the state change is
    // observable and attributable.
    expect(okBody.dispatch).toBe("failed");
    const [after] = await db.select().from(emailNotifications).where(eq(emailNotifications.id, row.id)).limit(1);
    expect(after.status).toBe("failed");
    expect(after.lastError).toBe("EMAIL_API_KEY_NOT_CONFIGURED");
    expect(after.attempts).toBe(2); // one dispatch before, one after the requeue
    ...
    expect(audits[0].details).toMatchObject({ releasedAbandonedClaim: false });

    // A `sent` row is terminal: requeueing it is refused (409).
    await db
...
/**
 * MEDIUM-2 — the operator path must ACTUALLY re-drive, and nothing may
 * re-drive automatically. These tests pin the four guarantees that make the
 * requeue meaningful instead of decorative:
 *   1. an injected transport receives the message and the row becomes `sent`;
 *   2. a crash/failure between the state change and the transport leaves the row
 *      RECOVERABLE (never lost, never silently re-sent);
 *   3. concurrent redispatches produce EXACTLY ONE transport call;
 *   4. a stranded claim can only be released explicitly and only after the
 *      documented abandonment threshold.
 */
describe("MEDIUM-2 — requeue re-drives for real (never automatically)", () => {
  async function unknownRow(): Promise<{ orderId: number; notificationId: number }> {
    const { orderId } = await createPendingOrder({ prefix: PREFIX });
    const notificationId = await queueNotification(orderId);
    await dispatchEmailNotification(notificationId, { fetchImpl: transport(503), apiKey: TEST_TRANSPORT_KEY });
    const [row] = await db.select().from(emailNotifications).where(eq(emailNotifications.id, notificationId)).limit(1);
    expect(row.status).toBe("delivery_unknown");
    return { orderId, notificationId };
  }

  it("sends the message through the injected transport after the requeue", async () => {
    const { notificationId } = await unknownRow();
    let calls = 0;
    const countedTransport = (async () => {
      calls += 1;
    ...
    const result = await redispatchRequeuedNotification({
      id: notificationId,
      actorId: (await createUser(PREFIX, "admin")).id,
      deps: { fetchImpl: countedTransport, apiKey: TEST_TRANSPORT_KEY },
    });
    expect(result).toMatchObject({ ok: true, code: "REQUEUED", dispatch: "sent" });
    expect(calls).toBe(1);
  ...
  it("leaves the row recoverable when the dispatch fails, and never re-sends by itself", async () => {
    const { notificationId } = await unknownRow();

    // The transport is unreachable → the dispatch is ambiguous → `delivery_unknown`.
    const result = await redispatchRequeuedNotification({
      id: notificationId,
      actorId: (await createUser(PREFIX, "admin")).id,
      deps: { fetchImpl: transport(502), apiKey: TEST_TRANSPORT_KEY },
    });
    expect(result).toMatchObject({ ok: true, dispatch: "delivery_unknown" });

    // Nothing sweeps it: an explicit sweep only picks `queued` rows.
    const sweep = await dispatchQueuedEmails(50, { fetchImpl: transport(200), apiKey: TEST_TRANSPORT_KEY });
    expect(sweep.attempted).toBe(0);
    const [row] = await db.select().from(emailNotifications).where(eq(emailNotifications.id, notificationId)).limit(1);
    expect(row.status).toBe("delivery_unknown");
  ...
  it("recovers a row whose dispatch never started (crash between the two steps)", async () => {
    const { notificationId } = await unknownRow();
    // Step 1 committed, step 2 never ran.
    const requeued = await requeueEmailNotification({ id: notificationId, actorId: (await createUser(PREFIX, "admin")).id });
    expect(requeued).toMatchObject({ ok: true, code: "REQUEUED" });
    const [stranded] = await db.select().from(emailNotifications).where(eq(emailNotifications.id, notificationId)).limit(1);
    expect(stranded.status).toBe("queued");

    // The explicit, bounded sweep is the recovery path (no cron drives it).
    const sweep = await dispatchQueuedEmails(50, { fetchImpl: transport(200), apiKey: TEST_TRANSPORT_KEY });
  ...
  it("releases a stranded claim only explicitly and only after the threshold", async () => {
    const { notificationId } = await unknownRow();
    const admin = await createUser(PREFIX, "admin");

    // Arm the row and hand it to a transport that then "crashed": the row stays
    // `dispatching` with a fresh claim.
    expect((await requeueEmailNotification({ id: notificationId, actorId: admin.id })).ok).toBe(true);
    const claim = await claimQueuedEmail(notificationId);
    expect(claim?.status).toBe("dispatching");
    expect(claim?.dispatchStartedAt).not.toBeNull();

    // A FRESH claim can only be released with the explicit flag, and even then the
    // threshold protects an in-flight dispatch.
    const tooFresh = await requeueEmailNotification({
      id: notificationId,
      actorId: admin.id,
      allowStrandedClaim: true,
    });
    expect(tooFresh).toMatchObject({ ok: false, code: "CLAIM_STILL_ACTIVE" });
    expect((await requeueEmailNotification({ id: notificationId, actorId: admin.id })).ok).toBe(false);

    // Age the claim past the abandonment threshold: now the operator can release it.
    await db
      .update(emailNotifications)
      .set({ dispatchStartedAt: new Date(Date.now() - STRANDED_CLAIM_MS - 60_000) })
      .where(eq(emailNotifications.id, notificationId));
    const released = await requeueEmailNotification({
      id: notificationId,
      actorId: admin.id,
      allowStrandedClaim: true,
    });
    expect(released).toMatchObject({ ok: true, code: "REQUEUED" });
    const [row] = await db.select().from(emailNotifications).where(eq(emailNotifications.id, notificationId)).limit(1);
    expect(row.status).toBe("queued");
    expect(row.dispatchStartedAt).toBeNull();
  });

  it("sends EXACTLY ONCE when two operators requeue concurrently", async () => {
    const { notificationId } = await unknownRow();
    const admin = await createUser(PREFIX, "admin");
    let calls = 0;
    const countedTransport = (async () => {
    ...
    const [a, b] = await Promise.all([
      redispatchRequeuedNotification({ id: notificationId, actorId: admin.id, deps: { fetchImpl: countedTransport, apiKey: TEST_TRANSPORT_KEY } }),
      redispatchRequeuedNotification({ id: notificationId, actorId: admin.id, deps: { fetchImpl: countedTransport, apiKey: TEST_TRANSPORT_KEY } }),
    ]);

    expect(calls).toBe(1);
    expect([a.dispatch, b.dispatch].filter((outcome) => outcome === "sent")).toHaveLength(1);
    expect([a.ok, b.ok].filter(Boolean)).toHaveLength(1);
    const [row] = await db.select().from(emailNotifications).where(eq(emailNotifications.id, notificationId)).limit(1);
    expect(row.status).toBe("sent");
    expect(row.attempts).toBe(2); // one delivery + exactly one re-drive
  });
});
