...
interface SettlementStepResult {
   ...
   *
   * A divergent payload detected inside the C8 window (see `processEupagoWebhook`)
   * has to be escalated by `escalateConcludedConflict`, which opens its OWN
   * transaction and therefore can never run nested inside the settlement one.
   * The transaction returns the evidence and the caller escalates POST-COMMIT —
   * the same pattern already used for the outbox dispatch.
   */
  readonly conflictEscalation?: {
    readonly persisted: WebhookEventRecord;
    readonly conflict: PayloadConflict;
  ...
  try {
    ...
    const committed = await db.transaction(async (tx) => {
      ...
      if (!claimed) {
        ...
        // ── F-1 (Cycle 4) — TOCTOU: the divergent-payload gate must be RE-RUN. ──
        //
        // The C2 gate above evaluated `registration.event`, a snapshot read
        // OUTSIDE this transaction. When that snapshot still said `pending` or
        // `failed` the gate was skipped — yet the delivery that won the claim can
        // have CONCLUDED the event while this one waited for the row lock.
        //
        // Answering from the re-read STATUS ALONE would acknowledge, as a plain
        // `duplicate`, an authenticated payload that materially diverges from the
        // one that was concluded: precisely the silent change of meaning C2
        // exists to prevent, reached through the C8 window instead of the
        // redelivery path. So the same comparison runs again, against the row
        // that really exists now, and `ignored` stays excluded exactly as above.
        //
        // The escalation cannot happen here (it opens its own transaction): the
        // tx returns a marker and the caller escalates POST-COMMIT.
        if (current.status === "processed" || isAnomalyWebhookEvent(current)) {
          const conflict = describePayloadConflict(current, event);
          if (conflict) {
            const fingerprint = conflictFingerprint(conflict);
            const recorded =
              typeof current.metadata?.eventConflict === "string"
                ? current.metadata.eventConflict
                : null;
            if (recorded !== fingerprint) {
              return {
                // Fail-closed placeholder: should the post-commit escalation not
                // run, the caller still never sees a plain `duplicate`.
                outcome: "payment_anomaly",
                code: "PROVIDER_EVENT_CONFLICT",
                notificationId: null,
                conflictEscalation: { persisted: current, conflict, fingerprint },
              } satisfies SettlementStepResult;
            }
            // This exact divergence is already on record: answer with the anomaly
            // that exists (never a plain duplicate, never a second anomaly row).
            return {
              outcome: "payment_anomaly",
    ...
    // ── F-1 (Cycle 4) — POST-COMMIT escalation of a conflict detected inside
    // the C8 window. `escalateConcludedConflict` opens its OWN transaction, so it
    // must never run nested inside the one that just committed; the transaction
    // only handed back the evidence. Its arbitration is conditional per conflict
    // fingerprint, so a concurrent twin can never duplicate the anomaly or the
    // audit row. Nothing financial was settled by the committed transaction: the
    // claim failed, therefore no payment, no stock and no order state changed.
    if (committed.conflictEscalation) {
      const escalation = committed.conflictEscalation;
      return escalateConcludedConflict(
